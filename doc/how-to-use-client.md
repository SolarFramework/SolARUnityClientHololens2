...
![Switch platform](res/how-to-use-client/switch-platform.PNG)
...
8. Compression Enabled: toggles compression of sensor images to PNG to save bandwidth when sending images to the cloud services.
9. Selected Sensor: pick a sensor camera. Currently only PV (color front facing camera) and LEFT_FRONT (one of the VLC grey scale tracking cameras available in Research Mode) are supported. Default values for intrinsics are editable, allowing a calibration step to refine the values for a particular device. The Reset button can restore default values.

## Control client execution

In order to start and stop the client, register a button `OnClick` event on `SolArCloudHoloLens2.ToggleSensorCapture()`.
...
The sample app only does what is previously described. But client also provides some callback methods in order to surface some information so that it does not completely act as a black box.
